...
router.post("/createChatRoom", async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < allChats.length; i++) {
      if (
        allChats[i].seller._id == receiver &&
        allChats[i].buyer._id == req.user._id
      ) {
        return await ChatRoom.updateOne(
          { _id: allChats[i]._id },
          {
            $push: { conversation: { senderId: req.user._id, message, offer } },
          }
        );
      }
    }
...
router.post("/sendreport", async (req, res) => {
  ...
  if (req.user._id == sellerid) {
    user = await User.updateOne(
      { _id: buyerid },
      { $push: { reports: { Reason: reason, details: messageR } } }
    );
  } else {
    user = await User.updateOne(
      { _id: sellerid },
...
router.get("/getOfferConversations", async (req, res) => {
  ...
  let userOffers = allOffers.filter(
    (x) => x.owner._id == req.user._id || x.seller._id == req.user._id
  );
  let checkedOffers = userOffers.map((x) => ({
    offers: x,
    isSeller: x.seller._id == req.user._id,
  }));
...
router.patch("/offertransport/:id", async (req, res) => {
  try {
      console.log(req.params.id)
    let offer = await Offer.updateOne(
      { _id: req.params.id },
      { $push: { transporter: req.user._id } }
    );
    let statue = await Offer.updateOne(
      { _id: req.params.id },
      { $set: { statue: "On the way" } }
    );
    let user = await User.updateOne(
        { _id: req.user._id },
        { $push: { yourOffers: req.params.id } }
      );
    res.status(200).json({ offer, statue , user });
  } catch (error) {
    console.log(error);
...
router.delete("/deleteOffer/:id", async (req, res) => {
  ...
  try {
    ...
    let user = await User.updateOne(
      { _id: owner },
      { $pull: { yourOffers: req.params.id } }
    );
    let slr = await User.updateOne(
      { _id: seller },
      { $pull: { yourOffers: req.params.id } }
    );
...
router.patch("/rejectOffer/:id", async (req, res) => {
  ...
  try {
    ...
    let chat = await ChatRoom.updateOne(
      { _id: chatId },
      { $pull: { conversation: { offer: req.params.id } } }
    );
...
router.patch("/confirmOffer/:id", async (req, res) => {
  let { chatId } = req.body;
  try {
    let statue = await Offer.updateOne(
      { _id: req.params.id },
      { $set: { statue: "Confirmed" } }
    );
    let chat = await ChatRoom.updateOne(
      { _id: chatId },
      { $pull: { conversation: { offer: req.params.id } } }
